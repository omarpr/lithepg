...
- **v1.0 local gate:** latest 2026-06-02 receipt on `main` at `d4f402a` passed full `swift test` plus seeded `script/dogfood_check.sh`; earlier package build and `script/package_verify.sh dist/LithePG.app` gates passed before any public release/tag publication.
...
- **Startup:** 134.11 ms shell readiness; 248.31 ms connected startup through seeded dogfood Postgres.
- **Query overhead:** 0.065 ms median overhead versus `psql` for `SELECT 1`; 0.051 ms for the dogfood query.
