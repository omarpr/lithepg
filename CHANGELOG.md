...
- [`script/v10_release_gate.sh`](script/v10_release_gate.sh) provides a fast v1.0 publication preflight for local branch/status/tag readiness, release-copy placeholder and SHA consistency checks, release artifact SHA validation, Homebrew cask shape checks, and required external publication inputs without printing secret/contact/tap or digest values.
