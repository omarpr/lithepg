...
def png_dimensions_are_valid(payload, expected_dimension):
    ...
    scanline_payload_lengths = png_expected_scanline_payload_lengths(
        width,
        height,
        bit_depth,
        color_type,
...
def png_expected_scanline_payload_lengths(width, height, bit_depth, color_type, interlace_method):
    ...
    for start_x, start_y, step_x, step_y in adam7_passes:
        ...
        scanline_payload_lengths.extend([row_bytes] * pass_height)
...
def png_idat_stream_is_valid(payload, scanline_payload_lengths, color_type, bit_depth):
    ...
    while offset < len(payload):
        ...
        if any(not (0x41 <= byte <= 0x5A or 0x61 <= byte <= 0x7A) for byte in chunk_type):
            return False
        if not (0x41 <= chunk_type[2] <= 0x5A):
            return False
        chunk_data_start = offset + 8
        chunk_crc_offset = chunk_data_start + chunk_length
        if chunk_crc_offset + 4 > len(payload):
            return False
        expected_crc = int.from_bytes(payload[chunk_crc_offset:chunk_crc_offset + 4], byteorder="big")
        ...
        if chunk_type[:1].isupper() and chunk_type not in {b"IHDR", b"PLTE", b"IDAT", b"IEND"}:
            return False
        if chunk_type in {b"tEXt", b"zTXt", b"iTXt", b"tIME", b"eXIf", b"pHYs", b"sBIT", b"bKGD", b"hIST"}:
            return False
        if chunk_type == b"IHDR":
            ...
            if color_type == 0:
                ...
                if not seen_plte:
                    return False
                if chunk_length == 0 or chunk_length > palette_entries:
                    return False
            else:
                return False
            seen_trns = True
            ...
            if seen_idat or seen_plte or seen_gama:
                return False
            if chunk_length != 4:
                return False
    ...
    if len(inflated) != expected_inflated_length:
        return False

    scanline_offset = 0
    for row_length in scanline_payload_lengths:
        if inflated[scanline_offset] > 4:
            return False
        scanline_offset += 1 + row_length
    return scanline_offset == len(inflated)
