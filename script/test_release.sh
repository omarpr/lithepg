...
assert_contains "$script_contents" './script/prepare_release_changelog.sh "$VERSION" "$PREPARED_CHANGELOG"'
assert_contains "$script_contents" '/bin/cp "$PREPARED_CHANGELOG" "$ROOT_DIR/CHANGELOG.md"'
assert_contains "$script_contents" 'git -C "$ROOT_DIR" add -- CHANGELOG.md README.md packaging/homebrew/lithepg.rb'
assert_not_contains "$script_contents" 'CHANGELOG.md must contain a ## [v$VERSION] release entry before releasing'
...
assert_contains "$script_contents" 'gh release download "$TAG"'
assert_contains "$script_contents" 'uploaded release artifact SHA-256 does not match'
assert_contains "$script_contents" 'gh release edit "$TAG" --repo "$LITHEPG_GITHUB_REPOSITORY" --draft=false --latest'
assert_not_contains "$script_contents" '--prerelease'
...
assert_contains "$script_contents" '/bin/cp "$CASK_PATH" "$TAP_DIR/Casks/lithepg.rb"'
assert_contains "$script_contents" 'brew style --cask Casks/lithepg.rb'
assert_contains "$script_contents" 'brew audit --online --strict --cask "$LITHEPG_HOMEBREW_TAP/lithepg"'
assert_not_contains "$script_contents" 'brew audit --new'
assert_not_contains "$script_contents" 'brew audit --new --strict --cask Casks/lithepg.rb'
assert_contains "$script_contents" 'git -C "$TAP_DIR" add -- Casks/lithepg.rb'
assert_contains "$script_contents" 'git -C "$TAP_DIR" commit -m "lithepg $VERSION"'
