...
/usr/bin/printf 'Cask preview base version: '
IFS= read -r VERSION
...
TAG="v$CASK_VERSION"
ASSET_NAME="LithePG-$CASK_VERSION.zip"
...
run_at_root ./script/update_release_readme.sh "$CASK_VERSION" preview
/usr/bin/printf '# LithePG %s\n\nUnsigned preview: this artifact is ad-hoc signed and is not notarized by Apple.\n\nmacOS requires manual approval in System Settings -> Privacy & Security.\n\nVerify the download with:\n\n```sh\nshasum -a 256 -c %s\n```\n' \
  "$TAG" "$CHECKSUM_NAME" >"$RELEASE_NOTES"
...
gh release create "$TAG" \
  "$ASSET_PATH#LithePG $CASK_VERSION for macOS (unnotarized preview)" \
...
/usr/bin/cmp -s "$CHECKSUM_PATH" "$DOWNLOADED_CHECKSUM" || \
  fail "uploaded preview checksum file does not match"
(
  cd "$(/usr/bin/dirname "$DOWNLOADED_ZIP")"
  /usr/bin/shasum -a 256 -c "$CHECKSUM_NAME" >/dev/null
) || fail "uploaded preview checksum verification failed"
gh release edit "$TAG" \
  --repo "$LITHEPG_GITHUB_REPOSITORY" \
  --draft=false \
  --prerelease \
