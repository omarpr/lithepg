...
if ! /usr/bin/env -u PERL5OPT -u PERL5LIB -u PERLLIB /usr/bin/perl -e '
  ...
  sub png_idat_stream_is_valid {
    ...
    while ($offset < length($payload)) {
      ...
      return 0 unless substr($chunk_type, 2, 1) =~ /[A-Z]/;
      ...
      return 0 if $chunk_type eq "tEXt" || $chunk_type eq "zTXt" || $chunk_type eq "iTXt" || $chunk_type eq "tIME" || $chunk_type eq "eXIf" || $chunk_type eq "pHYs" || $chunk_type eq "sBIT";
      if ($chunk_type eq "IHDR") {
        ...
        $seen_srgb = 1;
      } elsif ($chunk_type eq "gAMA") {
        return 0 if $seen_idat;
        return 0 if $seen_plte;
        return 0 if $seen_gama;
        return 0 unless $chunk_length == 4;
