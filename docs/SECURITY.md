...
- Saved-connection metadata is HMAC-signed with a per-connection key stored in the credential store. Listing connection names does not open those Keychain items. Before LithePG reads a saved password, connects, edits or deletes a connection, it validates that connection and refuses unsigned or tampered metadata so a local file edit cannot silently redirect a saved password to another host. Successfully read integrity keys and passwords are cached only in process memory for the rest of that app session to avoid repeated Keychain authorization dialogs.
