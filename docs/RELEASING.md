...
The helper defaults to version `1.0`; pass `--version <version>` only when checking a different public version. It is intentionally fast: it reports the current git branch/status, verifies local tag readiness (`v0.5` present and `v<version>` absent), blocks on a dirty working tree, scans release-publication text/templates for unresolved `REPLACE_WITH_*` placeholders, verifies placeholder-free release copy contains the approved SHA-256 as an exact digest token, verifies the configured local final public zip artifact path has basename exactly `LithePG.app.zip`, verifies that artifact exists as a non-symlink regular file, contains the top-level `LithePG.app` wrapper plus `LithePG.app/Contents/Info.plist` and `LithePG.app/Contents/MacOS/LithePGApp` as regular files, verifies the app executable entry preserves owner executable permission, verifies `LithePG.app/Contents/Resources/AppIcon.icns` is present exactly once as a regular file with safe mode bits and ICNS magic, verifies `LithePG.app/Contents/_CodeSignature/CodeResources` is present as a regular file, verifies the extracted app bundle passes `codesign --verify --strict --deep`, verifies `codesign --display --verbose=4` reports the exact expected code-signature identifier `dev.omarpr.lithepg`, verifies `codesign --display --verbose=4` reports the Hardened Runtime flag, contains no unexpected top-level archive entries outside `LithePG.app`, and matches the approved SHA-256, verifies the placeholder-free Homebrew cask token is `lithepg`, verifies `version` matches the requested `v<version>` release (for example, `version "1.0"` for `v1.0`), verifies its `url` matches the exact LithePG GitHub Release artifact path, verifies its `verified:` URL is `github.com/omarpr/lithepg/`, verifies its public metadata (`name`, `desc`, and `homepage`) matches LithePG, verifies its valid uninstall quit bundle ID gate is `uninstall quit: "dev.omarpr.lithepg"`, verifies its `app "LithePG.app"` stanza installs the expected app bundle, verifies its macOS requirement is `depends_on macos: ">= :sonoma"`, verifies its `zap trash:` stanza includes the expected local cleanup paths, runs a redacted `/usr/bin/ruby -c` syntax check on placeholder-free cask files, verifies its `sha256` matches that same approved digest, and does not contact `origin` by default. If a remote tag check is desired, opt in with `--check-remote` or `LITHEPG_CHECK_REMOTE_TAGS=1`; the remote tag check verifies `origin` still has the last public milestone tag (`v0.5`) and does not yet have `v<version>`. Remote/network failures are reported as unknown and do not block the fast check, but a confirmed missing `origin` `v0.5` tag blocks publication. It does **not** run the full Swift test, dogfood, package, signing, or notarization gates.
...
If the zip path basename is not exactly `LithePG.app.zip`, the configured zip path is a symlink, the zip path is missing, the zip cannot be inspected, the archive omits the top-level `LithePG.app` wrapper or either required bundle entry (`LithePG.app/Contents/Info.plist` and `LithePG.app/Contents/MacOS/LithePGApp`), either required bundle entry is a symlink or other non-regular file, the app executable entry lacks owner executable permission, `LithePG.app/Contents/Resources/AppIcon.icns` is missing, duplicated, non-regular, unsafe, or lacks ICNS magic, `LithePG.app/Contents/_CodeSignature/CodeResources` is missing or not a regular file, the embedded app code signature fails strict verification, the code-signature identifier is not `dev.omarpr.lithepg`, the code signature cannot be inspected for or lacks the Hardened Runtime flag, the archive contains any unexpected top-level entry outside `LithePG.app` (for example `README.txt`, `dist/LithePG.app/...`, `../evil`, `/absolute`, or another app/directory), the SHA-256 is missing/invalid, the computed `/usr/bin/shasum -a 256` value does not match, placeholder-free release copy omits the approved digest, or a placeholder-free cask has a missing/mismatched token, version, URL, verified URL, name, description, homepage, uninstall quit bundle ID, app stanza, macOS requirement, zap cleanup stanza, invalid Ruby syntax, or `sha256`, the fast preflight exits non-zero. Mismatch/invalid output is redacted and does not print the provided, expected, actual, release-copy digest, cask URL, cask metadata values, cask app stanza value, zap cleanup value, cask digest, signing identifier, symlink target, or archive contents.
