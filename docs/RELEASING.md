...
4. Confirm the cask metadata keeps `name "LithePG"`, `desc "Lean PostgreSQL client with local-first AI"`, and the valid uninstall quit gate `uninstall quit: "dev.omarpr.lithepg"`.
5. Confirm the cask installs `app "LithePG.app"`.
6. Confirm the `zap trash:` stanza includes both local cleanup paths: `~/Library/Application Support/LithePG` and `~/Library/Preferences/dev.omarpr.lithepg.plist`.
7. Run `ruby -c packaging/homebrew/lithepg.rb` for template syntax.
8. If Omar has provided the external tap target, copy the cask into that tap and run Homebrew checks there, for example `brew style --cask Casks/lithepg.rb` and `brew audit --cask --new --strict Casks/lithepg.rb`.
...
If the zip path is missing, the SHA-256 is missing/invalid, the computed `/usr/bin/shasum -a 256` value does not match, a placeholder-free cask has a missing/mismatched `version`, a placeholder-free cask has a missing/mismatched `url`, a placeholder-free cask has a missing/mismatched `verified:` URL, a placeholder-free cask has missing/mismatched `name`, `desc`, or `uninstall quit: "dev.omarpr.lithepg"` metadata, a placeholder-free cask has a missing/mismatched `app "LithePG.app"` stanza, a placeholder-free cask has a missing/mismatched `zap trash:` cleanup stanza, or a placeholder-free cask has a missing/mismatched `sha256`, the fast preflight exits non-zero. Mismatch/invalid output is redacted and does not print the provided, expected, actual, cask URL, cask metadata values, cask app stanza value, zap cleanup value, or cask digest.
