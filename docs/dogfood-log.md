...
## 2026-06-29 — v1.1 backlog start: result export (CSV + JSON serializers)

- Rationale: v1.0 is feature-complete and blocked only on Omar-gated external publication steps (signing/notary credentials, security contact, GitHub Release, `v1.0` tag), and the recent ticks had become a diminishing-returns spiral of release-gate artifact micro-hardening. Per the watchdog handoff ("after v0.5/v1.0 gate, start the next highest-value small task with tests"), began the roadmap's explicit v1.1+ backlog item *"Export formats beyond CSV (JSON, …)"*. The results grid already had a disabled Export button labeled "Export results lands in a later polish pass", so a headless, testable serializer is the natural first slice.
- Added `Sources/LithePGCore/ResultExporter.swift`: a pure, on-device `ResultExporter` enum exposing `csv(for:)`, `json(for:)`, an `export(_:as:)` dispatcher, and a `Format` enum (`.csv`/`.json`) with stable file extensions. It only reformats already-fetched `QueryResult` rows — no network, no credentials, no SQL execution.
  - CSV follows RFC 4180: header row of column names, CRLF record separator, comma field separator, fields containing comma/quote/CR/LF wrapped in double quotes with embedded quotes doubled, NULL cells rendered as empty fields, and an empty string returned for column-less command/empty results.
  - JSON emits a compact array of per-row objects keyed by column name in column order; NULL cells encode as literal `null`, text cells as JSON strings with full escaping of quotes, backslashes, `\n`/`\r`/`\t`/`\b`/`\f`, and other control characters as `\uXXXX`.
- Strict TDD: wrote 11 `ResultExporter` tests first in `Tests/LithePGCoreTests/ResultExporterTests.swift`.
  - RED verification passed as expected before the implementation existed: `swift test --filter ResultExporter` failed to compile with `cannot find 'ResultExporter' in scope`.
  - GREEN verification passed: `swift test --filter ResultExporter` reported all 11 tests in the suite passing, including a round-trip check that the emitted JSON decodes back to the original strings via `JSONSerialization`.
- Wider local verification passed: full `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test` now reports 138 tests across 21 suites (was 127 across 20), with no regressions. Focused added-line secret/injection static scan reported no findings; an independent reviewer subagent was dispatched on the diff before commit.
- Release-impact dogfood verification was not applicable: this is a non-release-impacting core library addition with no startup/binary/connection path change, and Docker is unavailable in this cron environment.
...
## 2026-06-29 — v1.1 backlog: Markdown table export format

- Continued the roadmap's explicit v1.1+ backlog item *"Export formats beyond CSV (JSON, …)"*. CSV and JSON serializers plus their results-grid Export menu wiring already landed; this slice adds a third, high-value-for-engineers format: a GitHub-flavored Markdown (GFM) table. The motivating use case is Omar pasting query results straight into GitHub PRs/issues/Slack. It is pure on-device string reformatting of already-fetched rows — no network, no credentials, no SQL execution, nothing auto-run.
- Added `ResultExporter.markdown(for:)` and a `Format.markdown` case (file extension `md`) in `Sources/LithePGCore/ResultExporter.swift`, wired through the existing `export(_:as:)` dispatcher.
  - Output is a GFM table: a header row of column names, a `| --- |` delimiter row, then one row per record, each cell space-padded inside pipes (`| value |`).
  - Cells escape backslash first then pipe (`|` → `\|`) and convert `CRLF`/`LF`/`CR` to `<br>` so a single multi-line value can never break the one-row-per-record table layout. NULL cells render as blank cells. A result with no columns yields `""` (consistent with the CSV serializer).
- Wired a third **Markdown (.md)** item into the results-grid Export menu in `Sources/LithePGApp/ResultsTable.swift`, updated the menu help text, and extended the exhaustive `contentType(for:)` switch (Markdown maps to the `md` UTType with a `.plainText` fallback) so the `NSSavePanel` offers the right type.
- Strict TDD:
  - RED verification passed as expected before the implementation existed: `swift test --filter ResultExporter` failed to compile with `type 'ResultExporter.Format' has no member 'markdown'`. Five new core tests (`markdownBasic`, `markdownEscaping`, `markdownNull`, `markdownNoRows`, `markdownNoColumns`) plus updated `formatMetadata`/`exportDispatch` were written first; matching app-layer tests (`exportFileNames`, `exportContent`) were extended for the new format.
  - GREEN verification passed: focused `swift test --filter ResultExporter` (16 tests) and `swift test --filter ResultsTablePresentation` (10 tests) both green.
- Wider local verification passed: `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift build` clean and full `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test` now reports 146 tests across 21 suites (was 141), with no regressions. Focused added-line secret/injection static scan reported no findings; an independent reviewer subagent was dispatched on the diff before commit.
- Release-impact dogfood verification was not applicable: this is a non-release-impacting core/UI addition with no startup/binary/connection path change, and Docker is unavailable in this cron environment.
- Evidence artifact: `screenshots/evidence/2026-06-29-result-export-markdown.svg`.
- v1.0 remains correctly gated: all v1.0 plan tasks implemented; only the GitHub Release draft and `v1.0` tag stay open, both awaiting Omar's explicit approval and external signing credentials. No signing identity, notarization, upload, Homebrew publication, GitHub Release, tag, cron changes, Telegram delivery, or external publication was attempted.

## 2026-07-02 — v1.1 backlog: Copy results as CSV/JSON/Markdown to clipboard

- Continued the roadmap's v1.1+ export-format backlog. The results grid already had a file **Export** menu (CSV/JSON/Markdown via `NSSavePanel`) and a plain **Copy** button that put fixed TSV on the clipboard. This slice adds a **"Copy as…"** clipboard action so Omar can paste a formatted table straight into a GitHub PR/issue/Slack without saving a file first — the motivating daily-driver use case for the Markdown format. It is pure on-device string reformatting of already-fetched rows: no network, no credentials, no SQL execution, nothing auto-run.
- Converted the results-grid Copy button into a split `Menu` (`primaryAction: copy` keeps the one-click TSV behavior) with items: **Copy (TSV)**, **Copy as CSV**, **Copy as JSON**, **Copy as Markdown**. The CSV/JSON/Markdown items are gated on `ResultsTablePresentation.canExport(result)` (row results with columns only), matching the file Export menu.
- Added `ResultsTablePresentation.clipboardContent(for:as:)` (reuses the existing on-device `ResultExporter.export(_:as:)`) and a private `copyAs(_:format:)` that writes the serialized string to `NSPasteboard.general`. Copy-as and file-export produce byte-for-byte identical output for the same format.
- Strict TDD:
  - RED verification passed as expected before the helper existed: `swift test --filter ResultsTablePresentation` failed to compile with `type 'ResultsTablePresentation' has no member 'clipboardContent'`.
  - GREEN verification passed: focused `swift test --filter ResultsTablePresentation` (11 tests) green, including a new `clipboardContent` test asserting parity with `ResultExporter.csv/json/markdown` and with `exportContent`.
- Wider local verification passed: `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift build` clean and full `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test` now reports 156 tests across 23 suites (was 155), with no regressions. Focused added-line secret/injection static scan reported no findings; an independent reviewer subagent was dispatched on the diff before commit.
- Release-impact dogfood verification was not applicable: this is non-release-impacting UI wiring with no startup/binary/connection path change, and Docker is unavailable in this cron environment.
- Evidence artifact: `screenshots/evidence/2026-07-02-copy-as-csv-json-markdown.svg`.
- v1.0 remains correctly gated: all v1.0 plan tasks implemented; only the GitHub Release draft and `v1.0` tag stay open, both awaiting Omar's explicit approval and external signing credentials (`LITHEPG_CODESIGN_IDENTITY`/`LITHEPG_NOTARY_PROFILE`). No signing identity, notarization, upload, Homebrew publication, GitHub Release, tag, cron changes, Telegram delivery, or external publication was attempted.

## 2026-07-02 — v1.1 backlog: EXPLAIN statement builder + QueryResult plan parse

- Follow-up to the `QueryPlan` EXPLAIN parser slice. That commit landed a pure JSON-string parser but deferred UI wiring; the two missing bridge pieces between "the user's query" and "a parsed plan" are (a) constructing the `EXPLAIN (FORMAT JSON)` statement from arbitrary user SQL and (b) pulling the plan JSON back out of the `QueryResult` that Postgres returns. Both are pure, fixture-testable functions and are the natural prerequisite for the future "Run EXPLAIN" toolbar affordance — mirroring how `ResultExporter` landed its serializers before any UI. No network, no credentials, no SQL execution, nothing auto-run.
- Added to `Sources/LithePGCore/QueryPlan.swift`:
  - `explainStatement(for:analyze:)` — a pure string transform that trims surrounding whitespace and a single trailing semicolon from the user's SQL and wraps it as `EXPLAIN (FORMAT JSON) <sql>`, or `EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON) <sql>` when `analyze: true` (which actually executes the query, so callers gate it on explicit intent). The produced statement is derived solely from the user's own SQL — no connection URL or credential is ever part of it.
  - `parse(explainResult:)` — reads the plan JSON out of the first cell of the first row of an EXPLAIN `QueryResult` (Postgres returns it as text in the `QUERY PLAN` column) and delegates to the existing `parse(explainJSON:)`. Empty results or a NULL first cell throw the new `ParseError.emptyResult` case rather than crashing.
- Strict TDD: wrote 4 new `QueryPlan` tests first in `Tests/LithePGCoreTests/QueryPlanTests.swift` (plain `EXPLAIN (FORMAT JSON)` wrapping + semicolon/whitespace trimming, `ANALYZE, BUFFERS` variant, parsing a plan from a single-cell `QueryResult`, and the empty/NULL-cell `emptyResult` error path).
  - RED verification passed as expected before the helpers existed: `swift test --filter QueryPlan` failed to compile with `incorrect argument label in call (have 'explainResult:', expected 'explainJSON:')` and `cannot convert value of type 'QueryResult' to expected argument type 'String'`.
  - GREEN verification passed: `swift test --filter QueryPlan` reported all 12 tests passing (8 prior + 4 new).
- Wider local verification passed: `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift build` clean and full `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test` now reports 168 tests across 24 suites (was 164), with no regressions. `git diff --cached` static secret/injection scan reported no findings; an independent reviewer subagent was dispatched on the diff before commit.
- Release-impact dogfood verification was not applicable: this is a non-release-impacting core-library addition with no startup/binary/connection path change, and Docker is unavailable in this cron environment.
- UI wiring (a "Run EXPLAIN" affordance and a plan-tree view) is intentionally deferred to a follow-up slice so this commit stays a focused, fully tested bridge.
- Evidence artifact: `screenshots/evidence/2026-07-02-explain-statement-and-result-parse.svg`.
- v1.0 remains correctly gated: all v1.0 plan tasks implemented; only the GitHub Release draft and `v1.0` tag stay open, both awaiting Omar's explicit approval and external signing credentials (`LITHEPG_CODESIGN_IDENTITY`/`LITHEPG_NOTARY_PROFILE`). No signing identity, notarization, upload, Homebrew publication, GitHub Release, tag, cron changes, Telegram delivery, or external publication was attempted.

## 2026-07-02 — v1.1 backlog: EXPLAIN plan parser (QueryPlan)

- Rationale: v1.0 stays gated only on Omar-approved external publication (signing/notary credentials, security contact, GitHub Release, `v1.0` tag), so per the watchdog handoff the tick advances the next highest-value small v1.1+ backlog item with tests. The last several ticks were all export-format work (CSV/JSON/Markdown serializers + copy/export UI); rather than continue that spiral, this tick starts a different, genuinely high-value backend-engineer helper from the roadmap's explicit v1.1+ list (`docs/superpowers/specs/2026-04-18-roadmap-design.md` §7): *"Light DBA helpers: `EXPLAIN (ANALYZE, BUFFERS)` visualization … Read-only, no mutation UI."* The natural first slice — mirroring how `ResultExporter` landed as a pure serializer before any UI wiring — is a pure, fixture-testable parser for `EXPLAIN (FORMAT JSON)` output.
- Added `Sources/LithePGCore/QueryPlan.swift`: a pure, on-device `QueryPlan` value type that parses Postgres `EXPLAIN (FORMAT JSON[, ANALYZE, BUFFERS])` payloads into a `Node` tree. It only reads already-fetched EXPLAIN JSON text — no network, no credentials, no SQL execution, nothing auto-run.
  - `parse(explainJSON:)` decodes the single-element top-level array, requires the `"Plan"` object (throwing `ParseError.missingPlan` otherwise and `ParseError.invalidJSON` for non-JSON input), and recursively parses nested `"Plans"` arrays into `children`.
  - Each `Node` carries node type, relation name/alias, startup/total cost, plan rows/width, and optional `ANALYZE` actual-time/rows/loops fields. Top-level `Planning Time`/`Execution Time` are captured; `analyzed` reports whether actual-time data is present.
  - Derived read-only views: `outline` (an indented `-> ` tree outline, e.g. `Hash Join (cost=1.50..45.20 rows=200)`), `nodeCount`, and `costliestNode` (highest total cost, pre-order tie-break). The rendered `outline` is derived solely from plan node fields, so no connection URL or password can appear in it — covered by an explicit privacy-receipt test.
- Strict TDD: wrote 8 `QueryPlan` tests first in `Tests/LithePGCoreTests/QueryPlanTests.swift` (plain single-node parse, nested ANALYZE tree with timing, plain + nested outline rendering, costliest-node selection, malformed-JSON error, missing-Plan error, and the no-secrets-in-outline receipt).
  - RED verification passed as expected before the type existed: `swift test --filter QueryPlan` failed to compile with `cannot find 'QueryPlan' in scope`.
  - GREEN verification passed: `swift test --filter QueryPlan` reported all 8 tests passing.
- Wider local verification passed: `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift build` clean and full `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test` now reports 164 tests across 24 suites (was 156 across 23), with no regressions. `git diff --check` clean; focused added-line secret/injection static scan reported no findings; an independent reviewer subagent was dispatched on the diff before commit.
- Release-impact dogfood verification was not applicable: this is a non-release-impacting core-library addition with no startup/binary/connection path change, and Docker is unavailable in this cron environment.
- UI wiring (a "Run EXPLAIN" affordance and a plan-tree view) is intentionally deferred to a follow-up slice so this commit stays a focused, fully tested parser.
- Evidence artifact: `screenshots/evidence/2026-07-02-query-plan-explain-parser.svg`.
- v1.0 remains correctly gated: all v1.0 plan tasks implemented; only the GitHub Release draft and `v1.0` tag stay open, both awaiting Omar's explicit approval and external signing credentials (`LITHEPG_CODESIGN_IDENTITY`/`LITHEPG_NOTARY_PROFILE`). No signing identity, notarization, upload, Homebrew publication, GitHub Release, tag, cron changes, Telegram delivery, or external publication was attempted.

## 2026-07-02 — v1.1 backlog: EXPLAIN plan-tree presentation seam (PlanTreePresentation)

- Fifth slice of the v1.1+ "Light DBA helpers: EXPLAIN visualization" backlog item (`docs/superpowers/specs/2026-04-18-roadmap-design.md` §7). Prior slices landed the pure `QueryPlan` EXPLAIN parser, the `EXPLAIN (FORMAT JSON)` statement/result bridge, and the `QueryPlan.displayRows` flatten model — all in `LithePGCore`, all explicitly deferring UI. Before a SwiftUI plan-tree view can exist it needs a headless, fixture-testable app-layer presentation helper that formats each `DisplayRow` into display strings and accessibility labels — mirroring exactly how `ResultsTablePresentation` backs the results table before its view. This tick adds that seam. It only reads already-parsed plan node fields — no network, no credentials, no SQL execution, nothing auto-run.
- Added `Sources/LithePGApp/PlanTreePresentation.swift` (module `LithePGAppUI`): a stateless `enum` of pure formatting helpers over `QueryPlan.DisplayRow` / `QueryPlan`:
  - `nodeLabel(for:)` → node type plus relation (`Seq Scan on orders`); `costDetail(for:)` → `cost=<startup>..<total> · rows≈<estimate>` or `nil` when the node has no cost bounds; `costShare(for:)` → the row's `costPercent` rounded to a whole-percent string (`66%`) or `nil`; `timing(for:)` → `actual <ms> ms · <rows> rows` only when ANALYZE data is present, else `nil`; `indent(for:)` → `indentStep (16pt) × depth`; `accessibilityLabel(for:)` → node label + cost share + a "most expensive node" phrase for the costliest row; `summary(for:)` → node count plus `Execution <ms> ms` when analyzed.
  - Every string is derived solely from parsed plan node fields, so no connection URL or password can appear — covered by an explicit privacy-receipt test asserting no `@`, `://`, or `password` fragments across all rendered fields.
- Strict TDD: wrote 8 `PlanTreePresentation` tests first in `Tests/LithePGAppTests/PlanTreePresentationTests.swift` (node label, cost detail incl. no-cost nil, cost share rounding incl. nil, ANALYZE timing incl. non-ANALYZE nil, depth-scaled indentation, accessibility label with costliest flag, plan summary, and the no-secrets receipt).
  - RED verification passed as expected before the type existed: `swift test --filter PlanTreePresentationTests` failed to compile with `cannot find 'PlanTreePresentation' in scope`.
  - GREEN verification passed: `swift test --filter PlanTreePresentationTests` reported all 8 tests passing. (The Swift Testing display-name filter matched 0 cases first; retrying with the type name ran the suite, per the known filter quirk.)
- Wider local verification passed: `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift build` clean and full `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test` now reports 180 tests across 25 suites (was 172 across 24), with no regressions. `git diff --cached --check` clean; focused added-line secret/injection static scan reported no findings; an independent reviewer subagent was dispatched on the staged diff before commit.
- Release-impact dogfood verification was not applicable: this is a non-release-impacting UI-module addition with no startup/binary/connection path change, and Docker is unavailable in this cron environment.
- The actual SwiftUI plan-tree view (a "Run EXPLAIN" affordance plus a list built on these helpers) remains intentionally deferred to a follow-up slice so this commit stays a focused, fully tested presentation seam.
- Evidence artifact: `screenshots/evidence/2026-07-02-plan-tree-presentation.svg`.
- v1.0 remains correctly gated: all v1.0 plan tasks implemented; only the GitHub Release draft and `v1.0` tag stay open, both awaiting Omar's explicit approval and external signing credentials (`LITHEPG_CODESIGN_IDENTITY`/`LITHEPG_NOTARY_PROFILE`). No signing identity, notarization, upload, Homebrew publication, GitHub Release, tag, cron changes, Telegram delivery, or external publication was attempted.

## 2026-07-02 — v1.1 backlog: EXPLAIN plan display rows (QueryPlan.displayRows)

- Follow-up to the `QueryPlan` EXPLAIN parser + statement/result-bridge slices. Those landed a pure plan-tree parser and the `EXPLAIN (FORMAT JSON)` statement/result plumbing but explicitly deferred UI. Before a SwiftUI plan-tree view can exist, it needs a headless, fixture-testable presentation model that turns the recursive `QueryPlan.Node` tree into a flat, indented, `Identifiable` row list — mirroring how `ResultExporter` serializers and `ResultsTablePresentation` landed before their views. This tick adds exactly that seam. It only reads already-parsed plan fields — no network, no credentials, no SQL execution, nothing auto-run.
- Added `QueryPlan.DisplayRow` (`Sendable, Equatable, Identifiable`) and the derived `QueryPlan.displayRows` computed property in `Sources/LithePGCore/QueryPlan.swift`:
  - Flattens the plan tree in pre-order into rows carrying a stable integer `id` (SwiftUI list key), `depth` (indentation level), the `node`, a `costPercent` (node `Total Cost` as a percentage of the root total cost, or `nil` when the root has no cost / a non-positive total), and an `isCostliest` flag.
  - Exactly one row is flagged `isCostliest`, matching the existing `costliestNode` (highest total cost, pre-order tie-break); a `costliestClaimed` guard keeps the flag singular even when two nodes are structurally equal, and it falls back to the root when no node carries a cost.
  - Derived solely from plan node fields, so no connection URL or password can appear — covered by an explicit privacy-receipt test.
- Strict TDD: wrote 4 new `QueryPlan` tests first in `Tests/LithePGCoreTests/QueryPlanTests.swift` (single-node plan -> one 100% costliest row; nested plan -> ids `[0,1,2,3]`, depths `[0,1,1,2]`, correct cost-share and a single costliest root; no-cost root -> `costPercent == nil` with costliest falling back to root; no-secrets-in-rows receipt).
  - RED verification passed as expected before the member existed: `swift test --filter QueryPlan` failed to compile with `value of type 'QueryPlan' has no member 'displayRows'`.
  - GREEN verification passed: `swift test --filter QueryPlan` reported all 16 tests passing (12 prior + 4 new).
- Wider local verification passed: `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift build` clean and full `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test` now reports 172 tests across 24 suites (was 168), with no regressions. `git diff --cached --check` clean; focused added-line secret/injection static scan reported no findings; an independent reviewer subagent was dispatched on the diff before commit.
- Release-impact dogfood verification was not applicable: this is a non-release-impacting core-library addition with no startup/binary/connection path change, and Docker is unavailable in this cron environment.
- UI wiring (a "Run EXPLAIN" affordance and a plan-tree view built on these rows) remains intentionally deferred to a follow-up slice so this commit stays a focused, fully tested presentation seam.
- Evidence artifact: `screenshots/evidence/2026-07-02-query-plan-display-rows.svg`.
- v1.0 remains correctly gated: all v1.0 plan tasks implemented; only the GitHub Release draft and `v1.0` tag stay open, both awaiting Omar's explicit approval and external signing credentials (`LITHEPG_CODESIGN_IDENTITY`/`LITHEPG_NOTARY_PROFILE`). No signing identity, notarization, upload, Homebrew publication, GitHub Release, tag, cron changes, Telegram delivery, or external publication was attempted.

## 2026-07-11 — Beta prep: live Neon verification, keychain fallback, secret audit, copy pass

- **Live Neon end-to-end (real cloud Postgres 17):** a disposable Neon project (`lithepg-beta-e2e`, deleted after the run) verified the shipping connector against Neon's proxy over verified TLS. CLI smoke `.build/release/lithepg --url postgres://…neon.tech/neondb?sslmode=require&channel_binding=require` returned `SELECT 1 → 1` on both the direct endpoint and the `-pooler` endpoint, using the current `ep-*.c-N.<region>.aws.neon.tech` host shape; three repeat runs stayed green. The gated live app-layer suite (`POSTGRES_TEST_URL=<neon-url> swift test --filter 'live|Live'`) passed 6/6: AppState connect + query render, schema introspection (2), saved-connection production-environment flow, reconnect and query history. Credentials were held in process env only and never written to disk or the repo.
- One live-suite assumption was fixed: the query-history test hardcoded `contains("localhost")` in the connection label; it now derives the expected host from `POSTGRES_TEST_URL`, so the suite runs against any live target.
- Two new `NeonConnectionProfile` unit tests pin the modern host shape (proxy-cell `c-N` label, `channel_binding=require` query param) for direct and pooled URLs; suite is now 7 tests.
- An initial CLI failure against Neon was traced to a stale prebuilt `.build/release/lithepg` artifact, not a code defect; fresh debug and release builds both connect. The CLI gained an env-gated `LITHEPG_DEBUG_ERROR=1` stderr line that prints the credential-redacted `String(reflecting:)` of connection errors for future field debugging.
- **Keychain hardening + first real-keychain tests:** plain `swift test` runners and unsigned builds get `errSecMissingEntitlement` (-34018) from the data-protection keychain. `KeychainCredentialStore` now falls back to the legacy login keychain for saves (same `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly` attribute) and treats the data-protection keychain as absent for reads/deletes in that case, so unsigned beta builds can save credentials instead of failing. `docs/SECURITY.md` documents the posture. A new gated suite (`LITHEPG_KEYCHAIN_TESTS=1`, 3 tests) round-trips save/load/overwrite/delete against the real keychain under a dedicated test service and cleans up after itself.
- **Full-history secret audit before open-sourcing:** `gitleaks git` scanned all 412 commits — no leaks. A manual pattern sweep of `git log -p --all` (passwords in URLs, private keys, AWS/GitHub/Slack/OpenAI/Neon token shapes) matched only test fixtures, redaction examples and localhost dogfood defaults. A history-wide added-file name scan found no `.env`/key/credential files. No history rewrite needed.
